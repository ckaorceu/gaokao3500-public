...
(function () {
  ...
  function signUp(email, pw, username) {
    var opts = {};
  ...
  function changePassword(currentPw, newPw) {
    ...
    return sb.auth.signInWithPassword({ email: user.email, password: currentPw }).then(function (r) {
  ...
  function loadAll() {
    if (!cloudEnabled()) {
      return Promise.resolve({ sr: localGet(SR_KEY), tricks: localGet(TRICK_KEY) });
    }
  ...
  function showOtpStep(email) {
    ...
    document.getElementById('authResend').onclick = function () {
      ...
      signUp(pendingReg.email, pendingReg.pw, pendingReg.uname).then(function () {
  ...
  function resetPassword(email) {
    ...
    return sb.auth.resetPasswordForEmail(email, {
      redirectTo: 'https://word.tutw.fun/reset.html'
    }).then(function (r) {
      if (r.error) throw r.error;
    });
  }
  ...
  function showReset() {
    box(
      ...
      '<button class="auth-btn primary" id="authSendReset">发送重置邮件</button>' +
  ...
  function doReset() {
    ...
    resetPassword(email).then(function () {
      msg.className = 'auth-msg ok';
      msg.textContent = '如果邮箱已注册，重置邮件已发送，请到邮箱查收（来自 mail@tutw.fun）。';
    }).catch(function (e) {
      msg.className = 'auth-msg err';
  ...
  function doChangePw() {
    ...
    changePassword(cur, nw).then(function () {
      msg.className = 'auth-msg ok';
  ...
  window.Sync = {
    ...
    resetPassword: resetPassword, changePassword: changePassword, usernameAvailable: usernameAvailable,
