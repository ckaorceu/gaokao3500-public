...
(function () {
  ...
  function renderEmailVerify() {
    ...
    sb.auth.getUser().then(function (r) {
      ...
      var verified = !!(u && (u.email_confirmed_at || u.confirmed_at));
      if (verified) {
        box2.innerHTML =
          '<div class="set-2fa-title">✉️ 邮箱验证</div>' +
          '<p class="set-note">当前邮箱已验证 ✓</p>';
        return;
      }
      box2.innerHTML =
        '<div class="set-2fa-title">✉️ 邮箱验证</div>' +
        '<p class="set-note">当前邮箱尚未验证，请查收我们发送的验证邮件完成验证。</p>' +
        '<button class="auth-btn primary" id="setResendVerify">重新发送验证邮件</button>';
      var btn = document.getElementById('setResendVerify');
      if (btn) btn.onclick = function () { sendVerifyEmail(btn); };
    }).catch(function (e) {
      box2.innerHTML = '<div class="set-2fa-title">✉️ 邮箱验证</div><p class="set-note err">' + (authErrMsg(e) || '加载失败') + '</p>';
    });
  }

  function sendVerifyEmail(btn) {
    if (!btn) return;
    btn.disabled = true; btn.textContent = '发送中…';
    var email = (currentUser() && currentUser().email) || '';
    return sb.auth.resend({ type: 'signup', email: email }).then(function (r) {
      if (r.error) throw r.error;
      if (typeof toast === 'function') toast('验证邮件已发送，请查收邮箱');
      renderEmailVerify();
    }).catch(function (e) {
      btn.disabled = false; btn.textContent = '重新发送验证邮件';
      if (typeof toast === 'function') toast('发送失败：' + (authErrMsg(e) || '请稍后重试'));
    });
  }
  ...
  window.Sync = {
    ...
    // 邮箱验证（详见 renderEmailVerify / sendVerifyEmail）
